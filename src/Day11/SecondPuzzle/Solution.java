...
public class Solution {
    public static void main(String[] args) {
        try {
            ...
            int stepsTaken = countSteps(list, START_STEP);
    ...
    public static int countSteps(List<List<Integer>> list, int stepsTaken) {
        ...
        boolean isAnyLessThan10 = false;
        for (int i = 0; i < list.size(); i++) {
            for (int j = 0; j < list.get(0).size(); j++) {
                int val = list.get(i).get(j);
                if (val <= 9) {
                    isAnyLessThan10 = true;
                } else {
                    list.get(i).set(j, 0);
                }
            }
        ...
        if (isAnyLessThan10) {
            return countSteps(list, stepsTaken + 1);
        } else {
            return stepsTaken;
        }
    }

    public static void increaseAdjacent(int row, int col, List<List<Integer>> list) {
        ...
        for (int i = -1; i <= 1; i++) {
            for (int j = -1; j <= 1; j++) {
                ...
                if (val > 9) {
                    ...
                    increaseAdjacent(currRow, currCol, list);
                } else {
                    list.get(currRow).set(currCol, val + 1);
                }
            }
